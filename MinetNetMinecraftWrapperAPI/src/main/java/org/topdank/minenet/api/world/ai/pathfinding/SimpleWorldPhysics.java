...
public class SimpleWorldPhysics implements WorldPhysics {
	...
	private static final BlockLocation[] surrounding = new BlockLocation[] {
		...
		// bottom y - 1
		new BlockLocation(-1, -1, 1), new BlockLocation(0, -1, 1), new BlockLocation(1, -1, 1), new BlockLocation(-1, -1, 0), new BlockLocation(0, -1, 0), new BlockLocation(1, -1, 0), new BlockLocation(-1, -1, -1),
		new BlockLocation(0, -1, -1), new BlockLocation(1, -1, -1),
		// top y + 1
		new BlockLocation(-1, 1, 1), new BlockLocation(0, 1, 1), new BlockLocation(1, 1, 1), new BlockLocation(-1, 1, 0), new BlockLocation(0, 1, 0), new BlockLocation(1, 1, 0), new BlockLocation(-1, 1, -1),
		new BlockLocation(0, 1, -1), new BlockLocation(1, 1, -1), };
	...
	public boolean canWalk(BlockLocation from, BlockLocation to) {
		...
		valid = valid && isEmpty(toX, toY + 1, toZ); // Block above must be non-solid
		...
		if (isEmpty(fromX, fromY - 1, fromZ))
			...
			&& (((toY < fromY) && (toX == fromX) && (toZ == fromZ))
					|| ((canClimb(from) && canClimb(to)) || (!canClimb(from) && canClimb(to)) || (canClimb(from) && !canClimb(to) && ((toX == fromX) && (toZ == fromZ) ? true : !isEmpty(toX, toY - 1, toZ)))) || !isEmpty(toX,
							toY - 1, toZ));
		...
		// System.out.println("block " + id + " at x:" + x2 + " y:" + y2 + " z:" + z2 + " canwalk: " + valid);
		return valid;
	}
	...
	public boolean canClimb(BlockLocation location) {
		...
		if (id == 106) { // Vines (which require an adjacent solid block)
			if (!isEmpty(location.getX(), location.getY(), location.getZ() + 1) || !isEmpty(location.getX(), location.getY(), location.getZ() - 1) || !isEmpty(location.getX() + 1, location.getY(), location.getZ())
					|| !isEmpty(location.getX() - 1, location.getY(), location.getZ()))
				return true;
		}
		return false;
	}
	...
	private boolean isEmpty(int x, int y, int z) {
		...
		// System.out.println("block " + id + " at x: " + x + " y: " + y + " z: " + z + " is " + b);
		return b;
	}
