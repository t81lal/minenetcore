...
public final class BoundingBox {
	...
	public static BoundingBox getBoundingBox(double minX, double minY, double minZ, double maxX, double maxY,
			double maxZ) {
		return new BoundingBox(Math.min(minX, maxX), Math.min(minY, maxY), Math.min(minZ, maxZ), Math.max(minX, maxX),
	...
	public BoundingBox include(double x, double y, double z) {
		return getBoundingBox(Math.min(minX, x), Math.min(minY, y), Math.min(minZ, z), Math.max(maxX, x),
				Math.max(maxY, y), Math.max(maxZ, z));
	}
	
	public double boundedShiftX(BoundingBox other, double offX) {
		if ((minY >= other.getMaxY()) || (maxY <= other.getMinY()) || (minZ >= other.getMaxZ())
				|| (maxZ <= other.getMinZ()))
			return offX;
	...
	public double boundedShiftY(BoundingBox other, double offY) {
		if ((minX >= other.getMaxX()) || (maxX <= other.getMinX()) || (minZ >= other.getMaxZ())
				|| (maxZ <= other.getMinZ()))
			return offY;
	...
	public double boundedShiftZ(BoundingBox other, double offZ) {
		if ((minX >= other.getMaxX()) || (maxX <= other.getMinX()) || (minY >= other.getMaxY())
				|| (maxY <= other.getMinY()))
			return offZ;
